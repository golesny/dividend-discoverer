...
router.get('/isin/list', (req, res, next) => {
  ...
  //group by isin
  db.raw("select isin.isin, isin.name, isin.sector, isin.currency, isin.symbol, isin.symbolcurrency, "+
         "(select max(dividend.date)  from `dividend` where isin.`isin` = dividend.`isin`) as latest_div, "+
